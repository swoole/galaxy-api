...
class CodeUpHandler extends AbstractHandler
{
    /**
     * 是否匹配当前handler.
     */
    ...
    {
        return $request->hasHeader('x-codeup-event');
    }
    ...
    {
        return $this->get('object_kind') === 'push';
    }

    public function isTagPush() : bool
    {
        return $this->get('object_kind') === 'tag_push' && $this->get('after') !== '0000000000000000000000000000000000000000';
    }
    ...
    public function getEvent() : ?string
    {
        if ($this->isPush()) {
            ...
            return $this->get('object_kind', null);
        }
    }
    ...
    {
        $parts = explode('/', $this->get('repository.homepage', ''));
        $partsLen = count($parts);

        return $partsLen == 1 ? $this->getProjectName() : sprintf('%s/%s', $parts[$partsLen - 2], $parts[$partsLen - 1]);
    }
    ...
    {
        return $this->get('repository.git_http_url', null);
    }
    ...
    {
        return $this->get('repository.git_ssh_url', null);
    }
